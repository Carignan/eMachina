...
module gearsbyteethanddistance(t1=13,t2=51, d=60, teethtwist=1, which=1)
{
	...
	if(which == 1)
	{
		...
		union()
		{
			...
			//DIFFERENCE:
			//shafthole
			translate([0,0,-TT-gear_shaft_h]) 
				cylinder(r=gear1_shaft_r, h=gear_h+gear_shaft_h+ST+gear_shaft_h);
			...
			//setscrew shaft #2
			translate([0,0,-TT-gear_shaft_h+gear1_setscrew_offset])
				rotate([0,90,0])
				cylinder(r=gear1_setscrew_r, h=g1p_r);
				
				
			//setscrew captive nut #1
			...
			//setscrew captive nut #2
			translate([(g1p_r)/2, 0, -TT-gear_shaft_h]) 
				translate([0,0,(gear1_captive_nut_r+gear1_setscrew_offset)/2])
					#cube([gear1_captive_nut_h, gear1_captive_nut_d, gear1_captive_nut_r+gear1_setscrew_offset+ST],center=true);
